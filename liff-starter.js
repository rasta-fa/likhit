...
function getP(){
    ...
    if (!tipe) {
        document.getElementById('textx').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: 'XZEM Liff\n\nline://app/1559608776-5aq0mYaK?type=text&text=ZEM%20Liff\n\ntype > text\ntext > ZEM Liff'
            }]).then(function () {
                liff.closeWindow();
            });
        });
        document.getElementById('imagex').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: 'XZEM Liff\n\nline://app/1559608776-5aq0mYaK?type=image&img=https://is.gd/urwe1g\n\ntype > image\nimg > Link (HTTPS)'
            }]).then(function () {
                liff.closeWindow();
            });
        });
        document.getElementById('videox').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: 'XZEM Liff\n\nline://app/1559608776-5aq0mYaK?type=video&ocu=https://is.gd/0Invtt&piu=https://is.gd/urwe1g\n\ntype > video\nocu > video url\npiu > preview image'
            }]).then(function () {
                liff.closeWindow();
            });
        });
        document.getElementById('audiox').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: 'XZEM Liff\n\nline://app/1634215658-wD7QNvA0?type=audio&link=https://platelets.fun/public/sounds/music.mp3\n\ntype > audio\nlink > audio link'
            }]).then(function () {
                liff.closeWindow();
            });
        });
        document.getElementById('stickerx').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: 'XZEM Liff\n\nWith Animation:\nline://app/1559608776-5aq0mYaK?type=sticker&stk=anim&sid=32128231&pkg=3099312\n\nNo Animation:\nline://app/1559608776-5aq0mYaK?type=sticker&stk=noanim&sid=32128231&pkg=3099312\n\ntype > sticker\nstk > anim / noanim\nsid > sticker id\npkg > packages id'
            }]).then(function () {
                liff.closeWindow();
            });
        });
                    ...
                    actions: [
                        {
                            type:"uri",
                            label:"Me",
                            uri:"line://app/1559608776-5aq0mYaK?type=profile"
                        }
                    ]
                }
            }]).then(function () {
